...
function shell(title: string, body: string, action?: { label: string; url: string }) {
  const button = action
    ...
    : ''
  return `<div style="background:#070909;color:#f5f1e8;font-family:Arial,sans-serif;padding:32px;line-height:1.6"><div style="max-width:620px;margin:auto"><p style="color:#d3a23e;font-weight:700">DECCAN WHEELS</p><h1 style="font-family:Georgia,serif;font-weight:500">${title}</h1>${body}${button}<p style="color:#8d8981;font-size:12px;margin-top:32px">Deccan Wheels, Road No. 12, Banjara Hills, Hyderabad</p></div></div>`
}
...
export function sendVerificationEmail(to: string, name: string, token: string) {
  ...
  return sendTransactionalEmail({
    to,
    subject: 'Verify your Deccan Wheels account',
    previewUrl: url,
...
export function sendPasswordResetEmail(to: string, name: string, token: string) {
  ...
  return sendTransactionalEmail({
    to,
    subject: 'Reset your Deccan Wheels password',
    previewUrl: url,
