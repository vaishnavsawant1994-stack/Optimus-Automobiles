...
export async function GET() {
  ...
  return NextResponse.json({ data: data.map((item) => ({ id: item.id, name: item.name, quote: item.quote, purchase: item.purchase || 'Deccan Wheels customer', location: item.location || 'Hyderabad', avatar: item.avatarUrl || 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=240&q=85', rating: item.rating, verifiedBuyer: item.verifiedBuyer })) })
}
